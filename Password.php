...
class Password implements Rule, DataAwareRule, ValidatorAwareRule
{
    ...
    public static function defaults($callback = null)
    {
        ...
        if (!is_callable($callback) && !$callback instanceof static) {
            throw new InvalidArgumentException('Pemanggilan kembali harus dapat dipanggil ulang atau sambungan dari ' . static::class);
        }
    ...
    public function passes($attribute, $value)
    {
        ...
        $validator = Validator::make(
            ...
            if ($this->mixedCase && !preg_match('/(\p{Ll}+.*\p{Lu})|(\p{Lu}+.*\p{Ll})/u', $value)) {
                $validator->errors()->add($attribute, ':attribute harus berupa minimal satu huruf besar and satu kata huruf kecil.');
            }

            if ($this->letters && !preg_match('/\pL/u', $value)) {
                $validator->errors()->add($attribute, ':attribute harus berupa minimal satu huruf.');
            }

            if ($this->symbols && !preg_match('/\p{Z}|\p{S}|\p{P}/u', $value)) {
                $validator->errors()->add($attribute, ':attribute harus berupa minimal satu simbol.');
            }

            if ($this->numbers && !preg_match('/\pN/u', $value)) {
                $validator->errors()->add($attribute, ':attribute harus berupa minimal satu angka.');
            }
        });
        ...
        if ($this->uncompromised && !Container::getInstance()->make(UncompromisedVerifier::class)->verify([
            ...
            return $this->fail(
                ':attribute muncul dalam kebocoran data. Tolong pilih :attribute yang berbeda.'
            );
        }
